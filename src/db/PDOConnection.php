...
use iboxs\db\exception\DbException;
use iboxs\db\exception\PDOException;
use iboxs\Model;
...
abstract class PDOConnection extends Connection
{
    ...
    protected function getFieldType(string $type): string
    {
        ...
        return match (true) {
            ...
            str_contains($type, 'real')  || str_contains($type, 'decimal')            => 'float',
            str_contains($type, 'int') || str_contains($type, 'serial') ||
            str_contains($type, 'bit') || str_contains($type, 'tinyint')              => 'int',
    ...
    {
        return $this->bindType[$type] ?? PDO::PARAM_STR;
    }
    ...
    public function aggregate(BaseQuery $query, string $aggregate, $field, bool $force = false)
    {
        ...
        $field = $aggregate . '(' . (!empty($distinct) ? 'DISTINCT ' : '') . $this->builder->parseKey($query, $field, true) . ') AS iboxs_' . strtolower($aggregate);
