...
{
    protected $selectSql = 'SELECT * FROM (SELECT iboxsphp.*, rownum AS numrow FROM (SELECT  %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%) iboxsphp ) %LIMIT%%COMMENT%';
